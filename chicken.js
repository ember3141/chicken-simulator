...
*/
const STARTING_CHICKENS = 0,
    CHILDHOOD = 255,
    DEATHAGE = 1000,
    GONEAGE = 500,
    PREGNANCYDURATION = 500,
    HATCHTIME = 100,
    FERTILITYRATE = 15,
    ENDURANCE_MIN = 50,
    ENDURANCE_MAX = 100,
...
function draw() {
    ...
    rect(foodbin.x - 20, foodbin.y, 50, 50);
    fill(150, 75, 0);
    rect(foodbin.x, foodbin.y, 50, 50);
    fill(175, 100, 0);
    rect(foodbin.x + 5, foodbin.y + 5, 40, 40);
    fill(255, 255, 255);
    textSize(15);
    stroke(0);
    text("Food\ncache", foodbin.x + 5, foodbin.y + 20);
    pop();
    chicktick();
    for (var s = 0; s < eggs.length; s++) {
        ...
        fill("rgba(0,0,0,0.2)");
        circle(eggs[s].x - 3, eggs[s].y, EGGSIZE);
        fill(255);
        circle(eggs[s].x, eggs[s].y, EGGSIZE);
        eggs[s].eggtime++;
    ...
    for (var v = 0; v < sawblade.length; v++) {
        ...
        if (m.d == true && m.x > (x - (w / 2)) && m.x < (x + (w / 2)) && m.y > y - (h / 2) && m.y < y + (h / 2)) {
            sawblade[v].held = true;
            m.full = "saw";
        }
        if (m.d == false) {
            sawblade[v].held = false;
            m.full = null;
        }
    ...
    for (var i = 0; i < c.length; i++) {
        ...
        for (var v = 0; v < sawblade.length; v++) {
            ...
            if (b.x > (x - (w / 2)) && b.x < (x + (w / 2)) && b.y > y - (h / 2) && b.y < y + (h / 2)) {
                c[i].live = false;
                push();
                fill("rgba(255,0,0," + random(0.2, 1) + ")");
                circle(b.x + ran(-3, 3), b.y + ran(-3, 3), 20);
                pop();
            }
        }
        ...
        if (b.live == false && b.x > foodbin.x && b.x < foodbin.x + 50 && b.y > foodbin.y && b.y < foodbin.y + 50) {

            b.deathtimer = DEATHAGE;
            food++;
...
function drawhouse() {
    push();
    fill("rgba(0,0,0,0.15)");
    rect(CX * 0.2, CY * 0.80, CX * 0.4, CY * 0.18);
    pop();

    push();
    fill("rgb(200,0,0)");
    rect(CX * 0.3, CY * 0.80, CX * 0.4, CY * 0.1);
    pop();

    push();
    fill(50);
    arc(CX * 0.5, CY * 0.80, 50, 50, 0, PI);
    pop();
